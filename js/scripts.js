  ...
  shopList.addEventListener('click', (evt) => {
    ...
    if (prod.some(pathItem => pathItem.classList && pathItem.classList.contains('shop__item'))) {
      ...
      buttonOrder.addEventListener('click', (evt) => {
        ...
        document.getElementById('prodId').value = prod[0].dataset['prodid'];
        document.getElementById('prodPrice').value = prod[0].lastElementChild.dataset['price'];

        if (inputs.every(inp => !!inp.value)) {
          evt.preventDefault();

          var ajaxData = $('.js-order').serialize();
          ...
          $.ajax({
            ...
            success: function (data) {
              ...
              if(res.error === true){
                ...
                $('.shop-page__end-title').text("Произошла ошибка");
              }
            }
          });
...
if (pageOrderList) {
  ...
  pageOrderList.addEventListener('click', evt => {
    ...
    if (evt.target.classList && evt.target.classList.contains('order-item__btn')) {
      ...
      let orderId = evt.target.dataset['order_id'];

      $.ajax({
        url: '/admin/ajax.php',
        method: 'post',
        data: {
          'action' : 'chenge_order_status',
          'orderID':  orderId
        },
        success: function (data) {
          var res = JSON.parse(data);
          if (status.classList && status.classList.contains('order-item__info--no') && res.error) {
...
if (addList) {
  ...
  button.addEventListener('click', (evt) => {
    ...
    if( typeof  $('#product-id').val() != 'undefined'){

      if($('.add-list__item--active img').length != 0){
        var imageArr = $('#product-photo').val().split('\\');
        var imageName = imageArr[imageArr.length - 1];
        var imageData = $('.add-list__item--active img').attr('src').split(",")[1];
      }else{
        var imageData = false;
        var imageName = false;
      }
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (res.error == '') {
            ...
            form.hidden = true;
            popupEnd.hidden = false;
            $('.shop-page__end-title').text(res.error);
          }
        }
      });
    }else{
...
if (document.querySelector('.shop-page')) {
    var inputMin = $('#min-price');
    var inputMax = $('#max-price');
    var  minPrice = inputMin.data('min_price');
    var  maxPrice = inputMax.data('max_price');
  ...
  $('.range__line').slider({
    ...
    stop: function(event, ui) {
      inputMin.val($('.range__line').slider('values', 0));
      inputMax.val($('.range__line').slider('values', 1));
